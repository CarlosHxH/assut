...
const StepperItem: React.FC<StepperItemProps> = ({
  ...
  const iconClasses =
    isCompleted || isActive
      ? "w-6 h-6 text-blue-600 lg:w-6 lg:h-6 dark:text-blue-300"
      : "w-6 h-6 text-gray-500 lg:w-6 lg:h-6 dark:text-gray-400";
  ...
  return (
    ...
    >
      <div className={`${baseClasses} ${activeClasses}`}>
        <div className={iconClasses + " text-center"}>{icon}</div>
      </div>
    </li>
  );
};
...
export function CustomButtons({ next, back, end }: {
  next?: (e: React.FormEvent<HTMLFormElement> | null) => void;
  back?: () => void;
  end?: boolean;
}) {
  return (
